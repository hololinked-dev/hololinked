...
def test_func_with_annotations_model():
...
def test_func_with_annotations_validation(args, kwargs, raises, exmsg):
    model = get_input_model_from_signature(func_with_annotations)
    if raises:
...
def test_func_with_missing_annotations_model():
...
def test_func_with_missing_annotations_validation(args, kwargs, raises, exmsg):
    model = get_input_model_from_signature(func_with_missing_annotations)
    if raises:
        with pytest.raises(raises) as ex:
            pydantic_validate_args_kwargs(model, args=args if args else (), kwargs=kwargs if kwargs else {})
...
def test_func_with_no_annotations_model_for_empty():
...
def test_func_with_no_annotations_validation(args, kwargs, raises, exmsg):
    model = get_input_model_from_signature(func_with_no_annotations, model_for_empty_annotations=True)
    if raises:
        with pytest.raises(raises) as ex:
            pydantic_validate_args_kwargs(model, args=args if args else (), kwargs=kwargs if kwargs else {})
...
def test_func_with_no_annotations_model_none():
    model = get_input_model_from_signature(func_with_no_annotations)
    assert model is None


def test_func_with_kwargs_model():
...
def test_func_with_kwargs_validation(args, kwargs, raises, exmsg):
    model = get_input_model_from_signature(func_with_kwargs)
    if raises:
        with pytest.raises(raises) as ex:
            pydantic_validate_args_kwargs(model, args=args if args else (), kwargs=kwargs if kwargs else {})
...
def test_func_with_annotated_kwargs_model():
...
def test_func_with_annotated_kwargs_validation(args, kwargs, raises, exmsg):
    model = get_input_model_from_signature(func_with_annotated_kwargs)
    if raises:
        with pytest.raises(raises) as ex:
            pydantic_validate_args_kwargs(model, args=args if args else (), kwargs=kwargs if kwargs else {})
...
def test_func_with_args_model_for_empty():
...
def test_func_with_args_validation(args, kwargs, raises, exmsg):
    model = get_input_model_from_signature(func_with_args, model_for_empty_annotations=True)
    if raises:
        with pytest.raises(raises) as ex:
            pydantic_validate_args_kwargs(model, args=args if args else (), kwargs=kwargs if kwargs else {})
...
def test_func_with_args_model_none():
    model = get_input_model_from_signature(func_with_args)
    assert model is None


def test_func_with_annotated_args_model():
...
def test_func_with_annotated_args_validation(args, kwargs, raises, exmsg):
    model = get_input_model_from_signature(func_with_annotated_args)
    if raises:
        with pytest.raises(raises) as ex:
            pydantic_validate_args_kwargs(model, args=args if args else (), kwargs=kwargs if kwargs else {})
...
def test_func_with_args_and_kwargs_model_none():
    model = get_input_model_from_signature(func_with_args_and_kwargs)
    assert model is None


# def test_func_with_args_and_kwargs_model_for_empty():
#     model = get_input_model_from_signature(func_with_args_and_kwargs, model_for_empty_annotations=True)
#     assert issubklass(model, BaseModel)
#     assert model.model_fields["args"].annotation == typing.Tuple
#     assert model.model_fields["kwargs"].annotation == typing.Dict[str, typing.Any]
#     assert len(model.model_fields) == 2
#     assert model.model_config["extra"] == "forbid"


# def test_func_with_annotated_args_and_kwargs_model():
#     model = get_input_model_from_signature(func_with_annotated_args_and_kwargs)
#     assert issubklass(model, BaseModel)
#     assert model.model_fields["args"].annotation == typing.List[int]
#     assert model.model_fields["kwargs"].annotation == typing.Dict[str, int]
#     assert len(model.model_fields) == 2
#     assert model.model_config["extra"] == "forbid"
