...
class OceanOpticsSpectrometer(Thing):
    ...
    status = String(readonly=True, fget=lambda self: self._status, doc="descriptive status of current operation")  # type: str

    serial_number = String(
        default=None, allow_None=True, doc="serial number of the spectrometer to connect/or connected"
    )  # type: str

    last_intensity = ClassSelector(
        default=None, allow_None=True, class_=Intensity, doc="last measurement intensity (in arbitrary units)"
    )  # type: Intensity
    ...
    reference_intensity = ClassSelector(
    ...
    def measure(self, max_count=None):
        try:
            ...
            self.logger.info(
                f"starting continuous acquisition loop with trigger mode {self.trigger_mode} & integration time {self.integration_time} in thread with ID {threading.get_ident()}"
            )
            loop = 0
            while self._running:
                ...
                if self._running:
                    ...
                    self.last_intensity = Intensity(value=_current_intensity, timestamp=timestamp)
