...
class MessageValidatorMixin(TestCase):
    ...
    def validate_request_message(self, request_message: RequestMessage) -> None:
        ...
        self.assertTrue(
            isinstance(request_message.id, UUID)
            or isinstance(UUID(request_message.id, version=4), UUID)
        )
    ...
    def validate_response_message(self, response_message: ResponseMessage) -> None:
        ...
        self.assertTrue(
            isinstance(response_message.id, UUID)
            or isinstance(UUID(response_message.id, version=4), UUID)
        )
    ...
    def validate_event_message(self, event_message: EventMessage) -> None:
        ...
        # check message ID is a UUID
        self.assertTrue(
            isinstance(event_message.id, UUID)
            or isinstance(UUID(event_message.id, version=4), UUID)
        )
        # check message length
...
class TestMessagingContract(MessageValidatorMixin):
    ...
    def test_2_response_message(self):
        ...
        self.assertEqual(
            Serializers.json.loads(response_message._bytes[3]), None
        )  # INDEX_BODY = 3
